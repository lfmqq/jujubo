...
CREATE TABLE `iot_product` (
  ...
  `device_type` varchar(20) DEFAULT 'sensor' COMMENT '设备类型：sensor/actuator/gateway',
  `protocol_type` varchar(20) DEFAULT 'mqtt' COMMENT '通信协议：mqtt/http/coap/tcp',
  `data_format` varchar(20) DEFAULT 'json' COMMENT '数据格式：json/custom',
...
INSERT INTO `iot_product` VALUES (1, '温湿度传感器', 'temp_humidity_v1', '工业级温湿度采集设备', 'sensor', 'mqtt', 'json', 1, NOW(), NOW());
INSERT INTO `iot_product` VALUES (2, '智能电表', 'smart_meter_v1', '三相智能电表，支持远程抄表', 'sensor', 'http', 'json', 1, NOW(), NOW());
INSERT INTO `iot_product` VALUES (3, '智能网关', 'gateway_v1', '边缘计算网关，支持多协议接入', 'gateway', 'tcp', 'custom', 1, NOW(), NOW());

DROP TABLE IF EXISTS `iot_device`;
CREATE TABLE `iot_device` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '设备ID',
  `device_name` varchar(100) NOT NULL COMMENT '设备名称',
  `device_key` varchar(100) NOT NULL COMMENT '设备标识（SN/MAC）',
  `device_secret` varchar(200) DEFAULT NULL COMMENT '设备密钥',
  `product_id` bigint(20) NOT NULL COMMENT '所属产品ID',
  `status` tinyint(4) DEFAULT 0 COMMENT '状态：0=未激活 1=在线 2=离线',
...
INSERT INTO `iot_device` VALUES (1, '1号温度传感器', 'SN-TH-001', 'abc123', 1, 1, NOW(), '安装在A车间', NOW(), NOW());
INSERT INTO `iot_device` VALUES (2, '2号温度传感器', 'SN-TH-002', 'abc456', 1, 2, DATE_SUB(NOW(), INTERVAL 2 HOUR), '安装在B车间', NOW(), NOW());
INSERT INTO `iot_device` VALUES (3, '1号电表', 'SN-MT-001', 'def789', 2, 1, NOW(), '主配电柜', NOW(), NOW());
INSERT INTO `iot_device` VALUES (4, '边缘网关#1', 'SN-GW-001', 'ghj012', 3, 1, NOW(), '厂区东侧', NOW(), NOW());
...
INSERT INTO `iot_device_data` VALUES (1, 1, 'temperature', '26.5', 'float', NOW(), NOW());
INSERT INTO `iot_device_data` VALUES (2, 1, 'humidity', '58.3', 'float', NOW(), NOW());
INSERT INTO `iot_device_data` VALUES (3, 2, 'temperature', '31.2', 'float', DATE_SUB(NOW(), INTERVAL 1 HOUR), DATE_SUB(NOW(), INTERVAL 1 HOUR));
INSERT INTO `iot_device_data` VALUES (4, 3, 'voltage', '220.5', 'float', NOW(), NOW());
INSERT INTO `iot_device_data` VALUES (5, 3, 'current', '15.8', 'float', NOW(), NOW());
INSERT INTO `iot_device_data` VALUES (6, 3, 'power', '3.48', 'float', NOW(), NOW());
...
CREATE TABLE `gen_table` (
  ...
  `function_name` varchar(100) DEFAULT '' COMMENT '功能名',
...
CREATE TABLE `sys_job` (
  ...
  `job_group` varchar(20) DEFAULT 'DEFAULT' COMMENT '任务组',
  `cron_expression` varchar(100) NOT NULL COMMENT 'cron表达式',
  `invoke_target` varchar(200) NOT NULL COMMENT '调用目标(beanName.method)',
  `status` tinyint(4) DEFAULT 1 COMMENT '状态：1=运行 0=暂停',
  `remark` varchar(500) DEFAULT '' COMMENT '备注',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`)
...
CREATE TABLE `sys_job_log` (
  ...
  `invoke_target` varchar(200) DEFAULT '' COMMENT '调用目标',
  `status` tinyint(4) DEFAULT 0 COMMENT '执行状态：0=成功 1=失败',
...
-- 菜单数据：IoT 物联网
-- =============================================
INSERT INTO `sys_menu` VALUES (39, 0, 'IoT物联网', '/iot', 'Layout', '', 0, 'Cpu', 10, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (40, 39, '产品管理', '/iot/product', 'iot/product/index', 'iot:product:list', 1, 'Goods', 1, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (41, 39, '设备管理', '/iot/device', 'iot/device/index', 'iot:device:list', 1, 'Monitor', 2, NOW(), NOW(), 1, 1, 1);

-- IoT 按钮权限
INSERT INTO `sys_menu` VALUES (42, 40, '产品新增', '', '', 'iot:product:add', 2, '', 1, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (43, 40, '产品编辑', '', '', 'iot:product:edit', 2, '', 2, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (44, 40, '产品删除', '', '', 'iot:product:remove', 2, '', 3, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (45, 41, '设备新增', '', '', 'iot:device:add', 2, '', 1, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (46, 41, '设备编辑', '', '', 'iot:device:edit', 2, '', 2, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (47, 41, '设备删除', '', '', 'iot:device:remove', 2, '', 3, NOW(), NOW(), 1, 1, 1);

-- =============================================
-- 菜单数据：代码生成
-- =============================================
INSERT INTO `sys_menu` VALUES (48, 0, '开发工具', '/tool', 'Layout', '', 0, 'Tools', 20, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (49, 48, '代码生成', '/tool/gen', 'tool/gen/index', 'tool:gen:list', 1, 'EditPen', 1, NOW(), NOW(), 1, 1, 1);

-- 代码生成按钮权限
INSERT INTO `sys_menu` VALUES (50, 49, '生成代码', '', '', 'tool:gen:code', 2, '', 1, NOW(), NOW(), 1, 1, 1);

-- =============================================
-- 菜单数据：定时任务
-- =============================================
INSERT INTO `sys_menu` VALUES (51, 32, '定时任务', '/job', 'monitor/job/index', 'monitor:job:list', 1, 'Clock', 2, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (52, 32, '调度日志', '/job/log', 'monitor/job/log', 'monitor:job:log', 1, 'Tickets', 3, NOW(), NOW(), 1, 1, 1);

-- 定时任务按钮权限
INSERT INTO `sys_menu` VALUES (53, 51, '任务新增', '', '', 'monitor:job:add', 2, '', 1, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (54, 51, '任务编辑', '', '', 'monitor:job:edit', 2, '', 2, NOW(), NOW(), 1, 1, 1);
INSERT INTO `sys_menu` VALUES (55, 51, '任务删除', '', '', 'monitor:job:remove', 2, '', 3, NOW(), NOW(), 1, 1, 1);

-- =============================================
-- 角色权限分配（超级管理员 admin 获得所有新菜单）
-- =============================================
INSERT INTO `sys_role_menu` VALUES (1, 39), (1, 40), (1, 41), (1, 42), (1, 43), (1, 44), (1, 45), (1, 46), (1, 47);
INSERT INTO `sys_role_menu` VALUES (1, 48), (1, 49), (1, 50);
INSERT INTO `sys_role_menu` VALUES (1, 51), (1, 52), (1, 53), (1, 54), (1, 55);
